...
describe('JobListItem (store)', () => {
  ...
  test('It should mount', () => {
    component = renderWithProvider(<JobListItem job={job} />)(store)
  ...
  test('It should call a function on click without image', () => {
    component = renderWithProvider(
      <JobListItem job={{ ...job, image: 'asdfa' }} />
    )(store)
    const clickable = component.getByTestId('clickable-image')
    fireEvent.click(clickable)
    expect(mockDispatchRealStore).toHaveBeenCalledWith(
  ...
  test('It should call a function on click on description', () => {
    component = renderWithProvider(<JobListItem job={job} />)(store)
    const clickable = component.getByTestId('clickable-description')
    fireEvent.click(clickable)
    expect(mockDispatchRealStore).toHaveBeenCalledWith(
      expect.objectContaining({
        type: '@@router/CALL_HISTORY_METHOD',
  ...
  test('should NOT show processing buttons and progress if there is no process', () => {
    component = renderWithProvider(<JobListItem job={job} />)(store)
    expect(
      screen.queryByTestId('job-item-processing-list')
  ...
  test('should NOT show processing buttons and progress if there are no scans', () => {
    ...
    component = renderWithProvider(<JobListItem job={localJob} />)(store)
  ...
  test('should display the open process icon when ready to process', () => {
    ...
    component = renderWithProvider(<JobListItem job={localJob} />)(
