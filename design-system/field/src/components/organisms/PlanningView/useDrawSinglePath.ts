...
import { addPointFromClick } from './addPointFromClick'

/**
 * Hook that handles drawing paths
 * @returns [IClickableOption[], number]
 */
const useDrawSinglePath = (
  tool: PlanningTools,
   ...
   */
  useEffect(() => {
    if (tool === PlanningTools.DRAW_PATH && myVRProvider && canvas) {
      const singletapHandler = (e: HammerInput) => {
        /** DRAWING */
        panning.current || addPointFromClick(myVRProvider, e.srcEvent, dispatch)
      }
...
export default useDrawSinglePath
