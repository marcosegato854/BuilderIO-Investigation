...
import style from 'components/dialogs/CreateNewJobType/CreateNewJobType.module.scss'
...
import { defaultProfileOptions } from 'components/dialogs/NewJobForm/options'
import { darkTheme } from 'utils/themes/mui'

export interface ICreateNewJobTypeProps {
  /**
   * ok button callback
   */
...
/**
 * CreateNewJobType description
 */
const CreateNewJobType: FC<ICreateNewJobTypeProps> = ({
  okButtonCallback,
  cancelButtonCallback,
}: PropsWithChildren<ICreateNewJobTypeProps>) => {
  const dispatch = useDispatch()
  const { t } = useTranslation()
  const jobTypes = useSelector(selectDataStorageJobTypes)
  const defaultProfileOptionsArray = defaultProfileOptions()
  ...
  const title = t('job_browser.new_jobtype_title', 'save jobtype')
  const text = t('job_browser.new_jobtype_text', 'save jobtype or not?')
  const cancelButtonLabel = t('job_browser.new_jobtype_cancel', 'dont save')
  const okButtonLabel = t('job_browser.new_jobtype_ok', 'save')
    ...
    >
      <ThemeProvider theme={darkTheme}>
        <Formik
          initialValues={{
            jobTypeName: '',
            jobTypeProfile: defaultProfileOptionsArray[0].value,
          }}
        ...
        >
          {({ submitForm, values, setFieldValue }) => (
            <Form className={style.form}>
              ...
              <div className={style.profileRow} data-testid="jobtype-profile">
                <Typography
                  variant="body1"
                  sx={{
                ...
                <OpenSelectbox
                  value={values.jobTypeProfile}
                  options={defaultProfileOptionsArray}
                  onChange={(value) => setFieldValue('jobTypeProfile', value)}
                />
              </div>
...
export default CreateNewJobType
